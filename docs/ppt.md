...
* **指标：** 计算 **MRR@10** (Mean Reciprocal Rank)。
* **逻辑：** 利用 MRR@10 衡量检索增益，引导模型优化 Query 的语义命中精度，确保目标文档排位靠前。
...
### 3. 优化期：7B 规模 (指标突破)
* **目标：** 进一步挖掘模型潜力，通过更精细的超参数微调，追求 MRR@10 指标的极限突破。
